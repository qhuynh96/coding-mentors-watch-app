...
io.on(RoomEvent_1.RoomEvent.connection, function (socket) {
  ...
  socket.on(RoomEvent_1.RoomEvent.CREATE_ROOM, function (_a) {
    ...
    socket.emit(RoomEvent_1.RoomEvent.CLIENT_GET_MSG, { msg });
    // broadcast an new Room except
    socket.broadcast.emit(RoomEvent_1.RoomEvent.CREATED_ROOM, {
      newRoom: newRoom,
      userId: userId,
    });
    // send back to room creator
    socket.emit(RoomEvent_1.RoomEvent.CREATED_ROOM, {
      newRoom: newRoom,
      userId: userId,
    });
  });
  socket.on(RoomEvent_1.RoomEvent.JOIN_ROOM, function (_a) {
    ...
    socket.broadcast.emit(RoomEvent_1.RoomEvent.JOINED_ROOM, {
      userId: userId,
      roomInfo: roomInfo,
    });
    // send back to participant
    socket.emit(RoomEvent_1.RoomEvent.JOINED_ROOM, {
      userId: userId,
      roomInfo: roomInfo,
    });
  });
...
app.get("/watch-app/user", function (req, res) {
